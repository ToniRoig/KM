$(document).ready(function(){
  ...
  function _resize(){
    ...
    $('#slidebox').css({'width': _width , 'height': _height});
    $('.content').css({'width': _width, 'height': _height});
  };
  ...
  _resize();
	...
	$('#mainBanner .next').click(function () {
		Slidebox('next','stop');
	});
	$('#mainBanner .previous').click(function () {
...
function Slidebox(slideTo,autoPlay){
  var animSpeed=1000,
      ...
      sliderWidth=$('#slidebox').width(),
      leftPosition=$('#slidebox .container').css("left").replace("px", "");
  ...
  if(slideTo=='next'){ 
    ...
    if(leftPosition==-slideboxTotalContent){
      $('#slidebox .container').animate({left: 0}, animSpeed, easeType);
      $('#slidebox .thumbs a:first-child').removeClass('thumb').addClass('selected_thumb');
      $('#slidebox .thumbs a:last-child').removeClass('selected_thumb').addClass('thumb');
    } else {
        $('#slidebox .container').animate({left: '-='+sliderWidth}, animSpeed, easeType);
      ...
      if(leftPosition=='0'){
        $('#slidebox .container').animate({left: '-'+slideboxTotalContent}, animSpeed, easeType);
      ...
      if(leftPosition!=-slide2){
        clearInterval(autoPlayTimer);
        $('#slidebox .container').animate({left: -slide2}, animSpeed, easeType);
